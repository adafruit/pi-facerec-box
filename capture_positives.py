...
def is_letter_input(letter):
    """Utility function to check if a specific character is available
    on stdin. Comparison is case-insensitive."""
    if select.select(
        [
            sys.stdin,
        ],
...
if __name__ == "__main__":
    ...
    files = sorted(
        glob.glob(
            os.path.join(
                config.POSITIVE_DIR, POSITIVE_FILE_PREFIX + "[0-9][0-9][0-9].pgm"
            )
        )
    )
    COUNT = 0
    ...
    while True:
        ...
        if box.is_button_up() or is_letter_input("c"):
            ...
            if result is None:
                print(
                    "Could not detect single face!  Check the image in capture.pgm"
            ...
            filename = os.path.join(
                config.POSITIVE_DIR, POSITIVE_FILE_PREFIX + "%03d.pgm" % COUNT
            )
