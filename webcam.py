...
Webcam device capture class using OpenCV. This class allows you to capture
a single image from the webcam, as if it were a snapshot camera.

This isn't used by the treasure box code out of the box, but is useful
to have if running the code on a PC where only a webcam is available.
The interface is the same as the picam.py capture class so it can be
used in the box.py code without any changes.
"""
...
CAPTURE_HZ = 30.0  # Rate at which the webcam will be polled for new images.

# pylint: disable=too-few-public-methods
class OpenCVCapture:
    """OpenCV capture object for webcam use."""
